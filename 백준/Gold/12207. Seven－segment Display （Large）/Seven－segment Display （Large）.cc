...
string test() {
    ...
    for (int turn_state = 0; turn_state < (1 << 7); turn_state++) {
        for (int start_num = 0; start_num < 10; start_num++) {
            bool is_can = true;
            for (int i = 0; i < inputs.size(); i++) {
                int now_num = (start_num - i + 100) % 10;
                for (int j = 0; j < 7; j++) {
                    if ((turn_state & (1 << j)) == 0) {
                        if (inputs[i][j] != '0') {
                            is_can = false;
                        ...
                        if (inputs[i][j] != num_state[now_num][j]) {
                            is_can = false;
                            break;
                        }
                    }
            ...
            if (is_can) {
                poss.push_back({ turn_state,start_num });
                //cout << turn_state << "," << start_num << "들옴.\n";
            }
        }
    }
    ...
    string ans3 = "";
    for (intint ans : poss) {
        int now_num = (ans.se - int(inputs.size()) + 100) % 10;
        ...
        for (int j = 0; j < 7; j++) {
            if ((ans.fi & (1 << j)) == 0) {
                ...
                ans2[j] = num_state[now_num][j];
            }
        }

        if (ans3 == "" || ans3 == ans2)ans3 = ans2;
        else return "ERROR!";
    }
