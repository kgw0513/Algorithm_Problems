...
void check(int height) {
    ...
    bool is_keep = false;
    for (int i = 1; i <= n; i++) {
        if (inputs[i] > height && !is_keep) {
            is_keep = true;
            sum++;
        }
        else if (inputs[i] <= height)is_keep = false;
    }
...
int main() {
    ...
    for (auto h : arr) {
        for (int i : h.se) {
            if (state[i - 1] == true && state[i + 1] == true)sum++; 
            if (state[i - 1] == false && state[i + 1] == false)sum--;
