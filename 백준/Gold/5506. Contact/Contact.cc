...
int main() {
    ...
    map<int, priority_queue<intint>>brr;
    for (auto h : arr) {
        brr[-h.se].push(h.fi);
    }

    vector<pair<int,vector<intint>>>ans;
    for (auto& h : brr) {
        ans.push_back({ -h.fi,vector<intint>() });
        while (h.se.size()) {
            ans.back().se.push_back(h.se.top());
            h.se.pop();
        }
    }
    ...
    for (int i = 0; i<min(n,int(ans.size())); i++) {
        ...
        for (int j = 0; j < ans[i].se.size(); j++) {
            cout << " ";
            for (int k = ans[i].se[j].fi-1; k>=0; k--) {
                cout << ((ans[i].se[j].se & (1 << k)) == 0 ? "0" : "1");
            }
        }
        cout << "\n";
    }
