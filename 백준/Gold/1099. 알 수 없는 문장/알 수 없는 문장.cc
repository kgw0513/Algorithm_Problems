    ...
    for (int i = 1; i < w.size(); i++) {
        for (string h : inputs) {
            ...
            for (int j = 0; j < h.size(); j++) {
                counting[w[i - int(h.size()) + j + 1] - 'a']--;
                if (h[j] == w[i - int(h.size()) + j + 1])continue;
                ans++;
            }

            for (int j = 0; j < 26; j++) {
                if (counting[j] != 0)ans = -1;
            }

            if (ans == -1)continue;
            dp[i] = min(dp[i], dp[i - int(h.size())] + ans);
        }
    }

    cout << (dp[w.size() - 1] >= nINF ? -1 : dp[w.size() - 1]);
}