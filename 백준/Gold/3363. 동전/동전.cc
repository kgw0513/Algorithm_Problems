...
int main() {
    ...
    intint ans = { -1,-1 };
    constexpr int default_weight = 100;
    for (int i = 1; i <= 12; i++) {
        for (int j = 0; j < 2; j++) {
            int weight = 1;
            if (j == 1)weight = 10000;
            ...
            for (int k = 0; k < 3; k++) {
                int a_sum = 0;
                int b_sum = 0;
                for (const int& a : arr[k][0]) {
                    a_sum += (a == i ? weight : default_weight);
                }
                for (const int& b : arr[k][1]) {
                    b_sum += (b == i ? weight : default_weight);
                }
                if (state[k] == '>' && a_sum > b_sum) {
                    continue;
                }
                if (state[k] == '<' && a_sum < b_sum) {
                    continue;
