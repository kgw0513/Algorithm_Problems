...
int main() {
    ...
    while (s <= e) {
        ll mid = (s + e) / 2;
        if (k > bubble_swap_count(brr, mid)) {
            s = mid + 1;
        }
        else {
            e = mid - 1;
    ...
    find_mid--; //해당 갯수만큼 돌리고 그다음 훑어보면 됨.

    k -= bubble_swap_count(brr, find_mid);
    brr = setting_bubble_sort(brr, find_mid);
    ...
    for (int i = 1; i < n; i++) {
        ...
        swap(brr[i - 1], brr[i]);
        if(k==0){
            cout<<min(num2val[brr[i - 1]], num2val[brr[i]])<<" "<<max(num2val[brr[i - 1]], num2val[brr[i]]);
            return 0;
        }
    }
    return -1;
