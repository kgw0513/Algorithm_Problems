...
map<int, pair<string,set<string>>>arr;
int main() {
    ...
    while (n--) {
        cin >> a >> b;
        if (arr[b].se.find(a) != arr[b].se.end())continue;
        if (arr[b].se.empty())arr[b].fi = a;
        arr[b].se.insert(a);
    }
    ...
    string ans = "";
    for (auto h : arr) {
        const set<string> h2 = h.se.se;
        if (h2.size() < check_count) {
            check_count = h2.size();
            ans = h.se.fi;
            ans_num = h.fi;
        }
