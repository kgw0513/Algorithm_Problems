...
int main() {
	...
	for (int i = 1; i < n; i++) {
		int a, b, c;
		cin >> a >> b >> c;
		line[c].push_back({ a,b });
	}

	ll ans = 0;
	for (int cost = 200; cost >= 1; cost--) {
		for (intint& h : line[cost]) {
			int a = h.fi;
			int b = h.se;

			a = find(a);
			b = find(b);
			if (a == b)continue; //어차피 이거 나올 가능성 X 이므로 고려 ㄴㄴ

			if (a > b)swap(a, b);
			ll a_count = group_count[a];
			ll b_count = group_count[b];
			ans += a_count * b_count * cost;
			{
