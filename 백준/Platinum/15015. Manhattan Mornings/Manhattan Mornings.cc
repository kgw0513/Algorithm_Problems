...
int main() {
    ...
    while (n--) {
        ll_ll w;
        cin >> w.fi >> w.se;
        if (!(min(s.fi, e.fi) <= w.fi && w.fi <= max(s.fi, e.fi)))continue;
        if (!(min(s.se, e.se) <= w.se && w.se <= max(s.se, e.se)))continue;
        inputs.push_back(w);
    }
    ...
    for (ll_ll& w : inputs) {
        ...
        ll p = upper_bound(ans.begin(), ans.end(), h) - ans.begin();
        if (p >= ans.size()) {
            ans.push_back(h);
        }
        else {
