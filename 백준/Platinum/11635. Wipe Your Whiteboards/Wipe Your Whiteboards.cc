...
int main() {
    ...
    while (t--) {
        ...
        ll2_ll h = xgcd(a, b);
        ll a_ans = c * h.fi.fi / h.se;
        ll b_ans = c * h.fi.se / h.se;
        
        ll a_plus = -b / gcd(a, -b);
        ll b_plus = a / gcd(a, -b);

        {
            ll a_h = (a_ans <= 0 ? (-a_ans / a_plus + (-a_ans % a_plus != 0)) : 0);
            ll b_h = (b_ans <= 0 ? (-b_ans / b_plus + (-b_ans % b_plus != 0)) : 0);
            ...
            while (a_ans <= 0 || b_ans <= 0) {
                a_ans += a_plus;
                b_ans += b_plus;
            }
        }
        {
            ll a_h = (a_ans > 0 ? (a_ans / a_plus) : 0);
            ll b_h = (b_ans > 0 ? (b_ans / b_plus) : 0);
